...
let lang = getLang();
let catalog = { branches: [], services: [], staff: [], categories: [] };
let category = "all";
...
function renderServices() {
  ...
  const items = catalog.services.filter(item => availableAtBranch(item) && item.active !== false && (category === "all" || item.categoryId === category) && (!query || `${item.nameAr} ${item.nameEn}`.toLowerCase().includes(query)));
